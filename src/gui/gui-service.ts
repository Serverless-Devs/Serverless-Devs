      ...
      if (url[osType]) {
        ...
        try {
          const tmpZipFile = path.join(
            S_GUI_BASE_PATH,
            `serverless-devs-tool-client-${osType}.zip`,
          );
          const pbo: ProgressBarOptions = {
            total: 100,
            width: 0,
  ...
  async start(update?: boolean) {
    ...
    if (!fs.existsSync(appInfo) || update === true) {
      if (update === true) {
        ...
        logger.info(
          "Check that you don’t have a gui client, download it now, it will take you about 1 minute.",
        );
      }
      ...
      logger.info(
        "The system has detected a new version of the GUI. You can use `s gui --update` to upgrade.",
      );
      // 输出更新信息 versions[2]
      try {
        const lang =
          (await handlerProfileFile({ read: true, filePath: "set-config.yml" })).locale || "en";
        logger.log("\n");
