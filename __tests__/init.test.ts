...
test('start-unzip-oss-v3@0.0.5', async () => {
  ...
  const res = spawnSync(s, ['init', '--project', template, '-d', dest, '-a', 'default', '--app-name', 'appName', '--parameters', '{"bucketName":"xiliu-hz-test","functionName":"oss-invoke-fc-c5y9","prefix":"src","processedDir":"dst","region":"cn-hangzhou","retainFileName":"false","roleArn":"acs:ram::1702981446385561:role/aliyunfcdefaultrole","triggerRoleArn":"acs:ram::1702981446385561:role/aliyunosseventnotificationrole"}']);
...
test('TripBookingOrderProcessing@0.0.8', async () => {
  ...
  const dest = path.join(cwd, template)
  const res = spawnSync(s, ['init', '--project', template, '-d', dest, '-a', 'default', '--app-name', 'appName', '--parameters', '{"flowName":"trip-booking-order-processing-flow-9pg3","fnfRoleArn":"acs:ram::1431999136518149:role/fnf-execution-default-role","functionName":"TripBookingOrderProcessing-9pg3-qdpjelpy","region":"cn-hangzhou","serviceName":"trip-booking-flow-9pg3","serviceRoleArn":"acs:ram::1431999136518149:role/aliyunfcdefaultrole"}']);
  const stdout = res.stdout.toString();
  console.log(stdout);
  expect(fs.existsSync(dest)).toBeTruthy();
});
