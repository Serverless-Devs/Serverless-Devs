...
import { logger, i18n, common } from '@serverless-devs-cli/util';
import { ConfigUpdateError, } from '@serverless-devs-cli/error';
...
export class UpdateManager {
  ...
  async updateManager(providerAlias: any, inputSecret: any) {
    this.userInformation = yaml.safeLoad(fs.readFileSync(this.filePath, 'utf8')) as ConfigMap;
      ...
      if (providerAlias.Provider && providerAlias.AliasName) {
        ...
        if (hasProviderAlia) {
          ...
          throw new ConfigUpdateError(
            'The provider or alias information is not found. You can obtain the key information through: s config get -l',
          );
        }
      }
    } else {
    ...
    for (let i = 0; i < this.promptList.length; i++) {
      ...
      tempAccess[tempName] =
        valueOfValueLength > 6
          ? tempAccess[tempName].slice(0, 3) +
          printn(valueOfValueLength - 6, '*') +
          tempAccess[tempName].slice(valueOfValueLength - 3, valueOfValueLength)
          : tempAccess[tempName];
  ...
  async userInputCheck(inputSecret: any, hiddenMapping?: any) {
    ...
    Object.keys(inputSecret).forEach(item => {
      if (!providerAccessFormatSecret.includes(item)) {
        throw new ConfigUpdateError('Please Input Right Secret Format: [{{providerAccessFormatSecret}}]', {
